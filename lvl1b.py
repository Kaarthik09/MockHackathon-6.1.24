...
def optimize_delivery(input_data):
    ...
    for i in range(len(tsp_path) - 1):
        ...
        if not graph.nodes[location]['visited']:
            ...
            # Check if adding the current location exceeds the capacity
            if current_capacity + weight > vehicle_capacity:
                # If the current path is not empty, add it to the output
                if current_path:
                    delivery_slots["v0"]["path" + str(path_count)] = current_path
                    path_count += 1
    ...
    # Add the last path to the output
    if current_path:
        delivery_slots["v0"]["path" + str(path_count)] = current_path
...
if __name__ == "__main__":
    input_file = r'C:\MockHackathon 6.1.24\Student Handout\Input data\level1b.json'
    with open(input_file, 'r') as f:
        input_data = json.load(f)

    optimized_delivery = optimize_delivery(input_data)

    output_file = 'level1b_output.json'
